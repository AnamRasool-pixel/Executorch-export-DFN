...
class ExportableStreamingTorchDF(nn.Module):
    ...
    ## checking tensors 
    def check_tensor_memory_formats(self, tensor: Tensor, tensor_name: str) -> None:
        tensor_name_tensor = torch.tensor(0)  # Dummy tensor to satisfy TorchScript
        if tensor.ndim == 4:
            is_contiguous = tensor.is_contiguous()
            is_channels_last = tensor.is_contiguous(memory_format=torch.channels_last)
            if not (is_contiguous or is_channels_last):
                print(f"Tensor {tensor_name} with shape {tensor.shape} is neither contiguous nor channels_last")
    ...
    def forward(self, 
        ...
        self.check_tensor_memory_formats(input_frame, "input_frame")
        self.check_tensor_memory_formats(analysis_mem, "analysis_mem")
        self.check_tensor_memory_formats(synthesis_mem, "synthesis_mem")
        self.check_tensor_memory_formats(rolling_erb_buf, "rolling_erb_buf")
        self.check_tensor_memory_formats(rolling_feat_spec_buf, "rolling_feat_spec_buf")
        self.check_tensor_memory_formats(rolling_c0_buf, "rolling_c0_buf")
        self.check_tensor_memory_formats(rolling_spec_buf_x, "rolling_spec_buf_x")
        self.check_tensor_memory_formats(rolling_spec_buf_y, "rolling_spec_buf_y")
        self.check_tensor_memory_formats(enc_hidden, "enc_hidden")
        self.check_tensor_memory_formats(erb_dec_hidden, "erb_dec_hidden")
        self.check_tensor_memory_formats(df_dec_hidden, "df_dec_hidden")
