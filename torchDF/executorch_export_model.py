...
class ExportableStreamingTorchDF(nn.Module):
    def __init__(self, fft_size, hop_size, nb_bands,
        ...
        #print("enc:")
        #print(self.enc)
        #print("ERB Conv0 Layer:")
        #print(self.enc.erb_conv0)
        #print("DF Conv0 Layer:")
        #print(self.enc.df_conv0)
    ...
    def band_unit_norm(xs: Tensor, band_unit_norm_state, alpha: float) -> Tuple[Tensor, Tensor]:
        ...
        # Replace linalg_vector_norm with equivalent operations
        xs_abs = torch.sqrt(torch.sum(xs ** 2, dim=-1, keepdim=True)) 
        new_band_unit_norm_state = torch.lerp(xs_abs, band_unit_norm_state, alpha)
        output = xs / new_band_unit_norm_state.sqrt()
    ...
    def is_apply_gains(self, lsnr: Tensor) -> Tensor:
        ...
        Returns:
            output: Tensor[Bool] - whether to apply gains or not
        """
        def true_fn(lsnr: Tensor) -> Tensor:
            return torch.ones_like(lsnr, dtype=torch.bool)

        def false_fn(lsnr: Tensor) -> Tensor:
            return torch.le(lsnr, self.max_db_erb_thresh) * torch.ge(lsnr, self.min_db_thresh)

        return torch.cond(self.always_apply_all_stages, true_fn, false_fn, (lsnr,))
    ...
    def unpack_states(self, states: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        erb_norm_state = torch.where(
            torch.all(erb_norm_state == 0), 
            new_erb_norm_state,
            erb_norm_state
        )
        band_unit_norm_state = torch.where(
            torch.all(band_unit_norm_state == 0),
            new_band_unit_norm_state,
            band_unit_norm_state
        )
    ...
    def forward(self, 
        ...
        def true_fn(current_spec, rolling_spec_buf_x, atten_lim_db):
            current_spec_copy = current_spec.clone()
            ...
            result = torch.lerp(current_spec_copy, spec_noisy, lim)
            return result.detach() 

        def false_fn(current_spec, rolling_spec_buf_x, atten_lim_db):
            current_spec_copy_false = current_spec.clone()
            return current_spec_copy_false.detach()

        new_current_spec = torch.cond(
            torch.abs(atten_lim_db) > 0, 
            true_fn, 
            false_fn, 
            (current_spec, rolling_spec_buf_x, atten_lim_db)
        )

        enhanced_audio_frame, new_synthesis_mem = self.frame_synthesis(new_current_spec, synthesis_mem)
...
class TorchDFPipeline(nn.Module):
    def __init__(
        ...
        # self.torch_streaming_model.enc.erb_conv0[0].weight = nn.Parameter(
        #     self.torch_streaming_model.enc.erb_conv0[0].weight.permute(0, 2, 3, 1)
        # )
        # # DF Conv0
        # self.torch_streaming_model.enc.df_conv0[0].weight = nn.Parameter(
        #     self.torch_streaming_model.enc.df_conv0[0].weight.permute(0, 2, 3, 1)
        # )
        # # DF Conv p
        # self.torch_streaming_model.df_dec.df_convp[0].weight = nn.Parameter(
        #     self.torch_streaming_model.df_dec.df_convp[0].weight.permute(0, 2, 3, 1))
    ...
    def forward(self, input_audio: Tensor, sample_rate: int) -> Tensor:
        ...
        device = 'cpu'
        states_full_len = self.torch_streaming_model.states_full_len
        states = torch.zeros(states_full_len, device=device)
        atten_lim_db = torch.tensor(0.0, device=device)
        print(atten_lim_db)
        print(chunked_audio[0].shape)
        print(states.shape)
        print(atten_lim_db.shape)
        
        example_input = (chunked_audio[0].to(device), states.to(device), atten_lim_db.to(device))
        #pre_autograd_aten_dialect = capture_pre_autograd_graph(self.torch_streaming_model, example_input)
        aten_dialect: ExportedProgram  = export(self.torch_streaming_model,example_input)
        torch.save(aten_dialect, 'aten_exported_model.pt')
        print("successful")
        edge_program = to_edge(aten_dialect)
        executorch_program = edge_program.to_executorch()
        with open("dfn_exe_try.pte", "wb") as file:
...
def main(args):
    ...
    #torch.save(torch_df.torch_streaming_model.state_dict(), 'modified_model.pth')
    #torch.save(torch_df.torch_streaming_model, 'new_modified_model.pth')
    #scripted_model = torch.jit.script(torch_df)
    #scripted_model.save("final_torch_df_model.pt")
    #print("Model saved")
